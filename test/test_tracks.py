...
class TestBasicTracks:
    ...
    def test_single_track_append_local(self, local_episode, temp_workspace):
        """Test appending single data points to a track."""
        with local_episode(prefix="test/track-test") as episode:
            for i in range(5):
    ...
    def test_single_track_append_remote(self, remote_episode):
        ...
        with remote_episode(prefix="test/track-test-remote") as episode:
            for i in range(10):
                episode.track("loss").append(value=0.5 - i * 0.05, epoch=i)

    def test_multiple_tracks_local(self, local_episode, temp_workspace):
        """Test tracking multiple different metrics."""
        with local_episode(prefix="test/multi-track") as episode:
            for epoch in range(5):
                episode.track("train_loss").append(value=0.5 - epoch * 0.1, epoch=epoch)
                episode.track("val_loss").append(value=0.6 - epoch * 0.1, epoch=epoch)
                episode.track("accuracy").append(value=0.7 + epoch * 0.05, epoch=epoch)
    ...
    def test_multiple_tracks_remote(self, remote_episode):
        ...
        with remote_episode(prefix="test/multi-track-remote") as episode:
            for epoch in range(3):
                episode.track("train_loss").append(value=0.4 - epoch * 0.1, epoch=epoch)
                episode.track("val_loss").append(value=0.5 - epoch * 0.1, epoch=epoch)


class TestBatchAppend:
    """Tests for batch appending track data."""

    def test_batch_append_local(self, local_episode, temp_workspace, sample_data):
        """Test batch appending multiple data points at once."""
        with local_episode(prefix="test/batch-track") as episode:
            result = episode.track("loss").append_batch(sample_data["track_data"])
    ...
    def test_batch_append_remote(self, remote_episode, sample_data):
        ...
        with remote_episode(prefix="test/batch-track-remote") as episode:
            result = episode.track("metrics").append_batch(sample_data["track_data"])
            assert result["count"] == 5

    def test_large_batch_append_local(self, local_episode, temp_workspace):
        """Test appending a large batch of data."""
        batch_data = [{"value": i * 0.01, "step": i} for i in range(1000)]

        with local_episode(prefix="test/large-batch") as episode:
            result = episode.track("metric").append_batch(batch_data)
...
class TestFlexibleSchema:
    ...
    def test_multi_field_tracking_local(self, local_episode, temp_workspace):
        """Test tracks with multiple fields per data point."""
        with local_episode(prefix="test/multi-field") as episode:
    ...
    def test_multi_field_tracking_remote(self, remote_episode, sample_data):
        ...
        with remote_episode(prefix="test/multi-field-remote") as episode:
            for data in sample_data["multi_metric_data"]:
                episode.track("combined").append(**data)

    def test_varying_schemas_local(self, local_episode, temp_workspace):
        """Test that schema can vary between data points."""
        with local_episode(prefix="test/varying-schema") as episode:
            episode.track("flexible").append(field_a=1, field_b=2)
            episode.track("flexible").append(field_a=3, field_c=4)
            episode.track("flexible").append(field_a=5, field_b=6, field_c=7)
...
class TestTrackMetadata:
    ...
    def test_track_metadata_creation_local(self, local_episode, temp_workspace):
        """Test that track metadata is created."""
        with local_episode(prefix="test/track-meta") as episode:
            for i in range(15):
                episode.track("metric").append(value=i * 0.1, step=i)
    ...
    def test_track_stats_local(self, local_episode):
        """Test getting track statistics."""
        with local_episode(prefix="test/track-stats") as episode:
            for i in range(20):
                episode.track("accuracy").append(value=0.5 + i * 0.02, step=i)

            stats = episode.track("accuracy").stats()
    ...
    def test_track_stats_remote(self, remote_episode):
        ...
        with remote_episode(prefix="test/track-stats-remote") as episode:
            for i in range(10):
                episode.track("loss").append(value=1.0 / (i + 1), step=i)
...
class TestTrackRead:
    ...
    def test_read_track_data_local(self, local_episode):
        """Test reading track data."""
        with local_episode(prefix="test/track-read") as episode:
            # Write data
            for i in range(20):
                episode.track("metric").append(value=i * 0.1, step=i)
    ...
    def test_read_with_pagination_local(self, local_episode):
        """Test reading track data with pagination."""
        with local_episode(prefix="test/track-page") as episode:
            # Write 100 data points
            for i in range(100):
                episode.track("metric").append(value=i, step=i)

            # Read first page
            page1 = episode.track("metric").read(start_index=0, limit=25)
            assert len(page1["data"]) == 25

            # Read second page
            page2 = episode.track("metric").read(start_index=25, limit=25)
    ...
    def test_read_track_data_remote(self, remote_episode):
        ...
        with remote_episode(prefix="test/track-read-remote") as episode:
            for i in range(15):
                episode.track("metric").append(value=i * 0.05, step=i)

            result = episode.track("metric").read(start_index=0, limit=5)
...
class TestListTracks:
    ...
    def test_list_all_tracks_local(self, local_episode):
        """Test listing all tracks in a episode."""
        with local_episode(prefix="test/track-list") as episode:
            episode.track("loss").append(value=0.5, step=0)
            episode.track("accuracy").append(value=0.8, step=0)
            episode.track("learning_rate").append(value=0.001, step=0)

            tracks = episode.track("loss").list_all()
    ...
    def test_list_all_tracks_remote(self, remote_episode):
        ...
        with remote_episode(prefix="test/track-list-remote") as episode:
            episode.track("metric1").append(value=1.0, step=0)
            episode.track("metric2").append(value=2.0, step=0)

            tracks = episode.track("metric1").list_all()
            assert len(tracks) >= 2
...
class TestTrackIndexing:
    ...
    def test_track_sequential_indices_local(self, local_episode, temp_workspace):
        """Test that track data points have sequential indices."""
        with local_episode(prefix="test/track-index") as episode:
            for i in range(10):
                episode.track("metric").append(value=i * 10)
    ...
    def test_track_indices_with_batch_local(self, local_episode, temp_workspace):
        """Test indices with batch append."""
        with local_episode(prefix="test/batch-index") as episode:
            batch1 = [{"value": i} for i in range(5)]
            batch2 = [{"value": i + 5} for i in range(5)]

            episode.track("metric").append_batch(batch1)
            episode.track("metric").append_batch(batch2)
...
class TestTrackEdgeCases:
    ...
    def test_empty_track_local(self, local_episode, temp_workspace):
        """Test episode with no tracks."""
        with local_episode(prefix="test/no-tracks") as episode:
            episode.log("No tracks created")
    ...
    def test_track_with_null_values_local(self, local_episode, temp_workspace):
        """Test tracking data with null values."""
        with local_episode(prefix="test/null-track") as episode:
            episode.track("metric").append(value=None, step=0, status="pending")
            episode.track("metric").append(value=0.5, step=1, status="complete")
    ...
    def test_track_with_special_characters_local(self, local_episode, temp_workspace):
        """Test track names with special characters."""
        with local_episode(prefix="test/special-track") as episode:
            episode.track("metric_1").append(value=1.0)
            episode.track("metric-2").append(value=2.0)
            episode.track("metric.3").append(value=3.0)

        tracks_dir = temp_workspace / "test" / "special-track" / "tracks"
        # Check that tracks were created (names may be sanitized)
        assert tracks_dir.exists()

    def test_very_frequent_tracking_local(self, local_episode, temp_workspace):
        """Test rapid, frequent tracking."""
        with local_episode(prefix="test/frequent-track") as episode:
            for i in range(1000):
                episode.track("metric").append(value=i * 0.001, step=i)
    ...
    def test_frequent_tracking_remote(self, remote_episode):
        ...
        with remote_episode(prefix="test/frequent-track-remote") as episode:
            for i in range(100):
                episode.track("metric").append(value=i * 0.01, step=i)

    def test_track_with_large_values_local(self, local_episode, temp_workspace):
        """Test tracking with very large numeric values."""
        with local_episode(prefix="test/large-values") as episode:
    ...
    def test_track_with_nested_data_local(self, local_episode, temp_workspace):
        """Test tracking with nested data structures."""
        with local_episode(prefix="test/nested-track") as episode:
            episode.track("metric").append(
                epoch=1,
                metrics={
    ...
    def test_track_name_collision_local(self, local_episode, temp_workspace):
        """Test multiple appends to same track name."""
        with local_episode(prefix="test/collision") as episode:
            episode.track("loss").append(value=1.0, epoch=0)
            episode.track("loss").append(value=0.9, epoch=1)
            episode.track("loss").append(value=0.8, epoch=2)
...
class TestTrackTimeQueries:
    ...
    def test_read_by_time_range_local(self, local_episode):
        """Test reading track data by time range."""
        import time

        with local_episode(prefix="test/track-time-range") as episode:
    ...
    def test_read_by_time_reverse_local(self, local_episode):
        """Test reading track data in reverse order."""
        import time

        with local_episode(prefix="test/track-time-reverse") as episode:
            base_time = time.time()
            for i in range(10):
                episode.track("metric").append(value=i, _ts=base_time + i)

            # Read in reverse
            result = episode.track("metric").read_by_time(reverse=True, limit=3)
    ...
    def test_read_by_time_no_range_local(self, local_episode):
        """Test reading all data without time range."""
        import time

        with local_episode(prefix="test/track-time-all") as episode:
            base_time = time.time()
            for i in range(5):
                episode.track("metric").append(value=i * 10, _ts=base_time + i)

            # Read all data
            result = episode.track("metric").read_by_time(limit=100)

        assert result["total"] == 5

    def test_read_by_time_with_limit_local(self, local_episode):
        """Test reading with limit applied."""
        import time

        with local_episode(prefix="test/track-time-limit") as episode:
            base_time = time.time()
            for i in range(100):
                episode.track("metric").append(value=i, _ts=base_time + i * 0.01)

            # Read with limit
            result = episode.track("metric").read_by_time(
    ...
    def test_read_by_time_start_only_local(self, local_episode):
        """Test reading from start_time to end."""
        import time

        with local_episode(prefix="test/track-time-start") as episode:
            base_time = time.time()
            for i in range(10):
                episode.track("metric").append(value=i, _ts=base_time + i)
    ...
    def test_read_by_time_end_only_local(self, local_episode):
        """Test reading from beginning to end_time."""
        import time

        with local_episode(prefix="test/track-time-end") as episode:
            base_time = time.time()
            for i in range(10):
                episode.track("metric").append(value=i, _ts=base_time + i)
    ...
    def test_timestamp_inheritance_across_tracks_local(self, local_episode):
        ...
        with local_episode(prefix="test/timestamp-inherit") as episode:
            ...
            pose_data = episode.track("robot/pose").read_by_time(limit=10)
            image_data = episode.track("camera/left").read_by_time(limit=10)
            velocity_data = episode.track("robot/velocity").read_by_time(limit=10)
